...
public class ThoriumOreBlock extends Mindustry4craftModElements.ModElement {
	@ObjectHolder("mindustry4craft:thorium_ore")
	public static final Block block = null;
	public ThoriumOreBlock(Mindustry4craftModElements instance) {
		super(instance, 28);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("thorium_ore", "thorium_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 5)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(7, 0, 0, 64))));
		}
	}
}
